...
#### 推奨: Option A（フロントエンド実装）

**理由**:
- 3ヶ月の開発期間を考慮すると、フロントエンド実装が最も現実的
- Kuromoji.jsはブラウザ/React Nativeで動作可能
- Cloud Functionsのセットアップ・デバッグ時間を節約
- 評価実験時のネットワーク依存を削減
...
// 翻訳時に使用
const translateMenuItem = async (text, targetLang) => {
  if (!tokenizer) {
...
**スケジュール上の配置**:
- Week 6（形態素解析実装）: フロントエンドでKuromoji.jsセットアップ（3時間）
- Week 6（辞書マッチング実装）: シンプルマッチングも同時実装（2時間）
- Week 10（評価実験）: Level 1とLevel 2の精度比較
...
| **翻訳API** | DeepL API / Google Translate API | 機械翻訳 |
...
const findSpecializedTerms = async (tokens) => {
  ...
  tokens.forEach(token => {
    const match = dictionary.find(entry =>
      entry.term_ja === token.surface_form
    );
...
```javascript
[
  {original: "炙り", translation_en: "seared", category: "cooking_method", priority: 2},
  {original: "サーモン", translation_en: "salmon", category: "ingredient", priority: 2},
  {original: "寿司", translation_en: "sushi", category: "dish_name", priority: 1}
]
```
...
#### Google Translate API使用例（フォールバック）
...
```javascript
const translateText = async (text, targetLang, context) => {
  const useDeepL = true; // 設定で切り替え可能

  if (useDeepL) {
    try {
      return await translateWithDeepL(text, targetLang, context);
    } catch (error) {
      console.log('DeepL failed, fallback to Google Translate');
      return await translateWithGoogle(text, targetLang);
    }
  } else {
    return await translateWithGoogle(text, targetLang);
...
const postProcessTranslation = (translatedText, foundTerms, targetLang) => {
  ...
  foundTerms.forEach(term => {
    const expectedTranslation = targetLang === 'en'
      ? term.translation_en
      : term.translation_zh;
...
const handleTranslationError = (originalText, targetLang) => {
  // フォールバック: 辞書のみで翻訳
  const dictionaryOnlyTranslation = translateWithDictionaryOnly(originalText, targetLang);
...
exports.translateText = functions.https.onCall(async (data, context) => {
  ...
  if (!['en', 'zh'].includes(targetLang)) {
    throw new functions.https.HttpsError('invalid-argument', 'Unsupported target language');
  }
  ...
  try {
    ...
    // Step 4: 後処理
    const correctedText = postProcessTranslation(translatedText, foundTerms, targetLang);
...
exports.batchTranslateMenu = functions.https.onCall(async (data, context) => {
  ...
  for (const doc of menuSnapshot.docs) {
    ...
    // 名前を翻訳
    const translatedName = await translateTextInternal(menuItem.name_ja, targetLang);
...
const detectFrequentUnknownTerms = async () => {
  const logs = await db.collection('translation_logs')
